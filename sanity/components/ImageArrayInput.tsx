      ...
      client
        ...
        .then((asset: any) => {
          if (asset?.originalFilename) {
            ...
            // Always sync from asset metadata so re-uploaded files (same content, new name)
            // get correct title/color after our custom asset source patches originalFilename
            const patches: any[] = []
            if (title) patches.push(set(title, [index, 'title']))
            if (color) patches.push(set(color, [index, 'color']))
